...
//操作成功提示框关闭提示框
Dialog.alertInfoCol = function(msg) {
...
//操作成功提示框关闭提示框并且关闭输入框iFrame
Dialog.alertInfoColseAll = function(obj,msg) {
    // var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
    layer.alert(msg, {
        icon : 1
    },function(index){
...
//警告提示框要关闭弹窗
Dialog.alertWarnCol = function(msg) {
    var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
	layer.alert(msg, {
		icon : 7
	},function(){
...
//刷新table
Dialog.refreshTable = function(tableId) {
    $('#' + tableId).DataTable().draw();
}
...
Dialog.getCheckeBoxVal = function(checkBoxId) {
    return $('input[name=checkBoxId]:checked').val();
}