...
function cascade(options) {
    var defaults = {
        ...
        bindCols: ["id", "id", "id", "id"],
        selectNames: ["provinceId", "cityId", "areaId", "townId"],
        requestIds: ["pid", "pid", "pid", "pid"],
        requestNames: ["name", "name", "name", "name"],
        selectCode: ["id", "id", "id", "id"],
        selectClasses: ["validate[required]", "validate[required]", "validate[required]", "validate[required]"],
        selectMultiple: [true, true, true, true],
        bindData: null,
        backFun: null,
        frozenSelect: true
    };
    ...
    if ($opt_div.find("select").length === 0) {
        for (i = 0; i < opt.totalSelect; i++) {
            ...
            var select = '<select ' + (opt.selectMultiple[i] ? "" : "multiple") + '   data-cascade-val="' + bindValue + '" data-area-index=' + i + ' class="selectUp selectpicker linkage ' + opt.selectClasses[i] + '" data-live-search="true" name=' + opt.selectNames[i] + '>' + blankSelect + '</select>';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (index, item) {
                    ...
                    html += '<option data-area-code=' + item[_idPosition] + ' value=' + item.id + ' >' + item.name + '</option>';
                    areaIds.push(item[_idPosition]);
                });
                $this.html(html);
